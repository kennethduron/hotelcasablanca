      ...
      <section className="bg-hotel-ivory py-16">
        <div className="hotel-container">
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {destinations.map((destination) => <article className="overflow-hidden rounded-[8px] border border-hotel-line bg-white shadow-hotel-soft" key={destination.id}><Image alt={destination.name} className="h-44 w-full object-cover" height={220} src={destination.image} width={420} /><div className="p-5"><h3 className="hotel-serif text-2xl font-bold text-hotel-forest">{destination.name}</h3><p className="text-sm text-hotel-muted">{destination.location}</p><p className="mt-3 text-sm leading-7 text-hotel-muted">{destination.description}</p><div className="mt-4 flex justify-between text-sm"><strong>{destination.distance}</strong><strong>{destination.duration}</strong></div><LinkButton className="mt-5 w-full" href="/entorno" variant="forest">Ver detalles / Ver ruta</LinkButton></div></article>)}
          </div>
        </div>
      </section>
    </main>
